...
#define DEVICE_ADDRESS  0x0A /* Addresses 0x00 and 0xFF are broadcast */
#define NEIGHBOR_ADDRESS 0x08 /* Address of the associated device */
/***************************************************************************** */
/* Pins configuration */
...
volatile uint8_t buffer_to_send[4];
...
#endif*/
	if (cc_ptr < BUFF_LEN && c != '\n' && c != '\r') {
		//cc_tx_buff[cc_ptr++] = c;
		buffer_to_send[cc_ptr++] = c;
	} else if (cc_ptr >= BUFF_LEN && (c != '\n' || c != '\r')){
		cc_ptr = 0;
	}
	if ((c == '\n') || (c == '\r')) {
		cc_tx = 1;
	}
	//uprintf("DATA: %d, %d, %d", buffer_to_send[0], buffer_to_send[1], buffer_to_send[2]);
}
...
void send_on_rf(void)
{
	...
	uint8_t cc_tx_data[sizeof(buffer_to_send)+3];
	cc_tx_data[0]=sizeof(buffer_to_send)+2;
	cc_tx_data[1]=NEIGHBOR_ADDRESS;
	cc_tx_data[2]=DEVICE_ADDRESS;
	/*int i;
	for(i = 0; (i < 4 && buffer_to_send[i] != '\0'); i++)
        buffer_to_send[i] = buffer_to_send[i] + 3;*/

	memcpy(&cc_tx_data[3], &buffer_to_send, sizeof(buffer_to_send));
	//uprintf(UART0, "ok");
	/* Send */
	...
	int ret = cc1101_send_packet(cc_tx_data, sizeof(buffer_to_send)+3);
}
